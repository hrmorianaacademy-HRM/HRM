...
export function useWebSocket(isAuthenticated: boolean = false) {
  ...
  const connect = () => {
    if (!isAuthenticated) {
      console.log('WebSocket: Skipping connection - not authenticated');
      return;
    }
    try {
        ...
        try {
          ...
          switch (message.type) {
            case 'lead_created':
              ...
              queryClient.invalidateQueries({ queryKey: ["/api/my/accounts-pending"] });
